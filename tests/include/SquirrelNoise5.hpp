...
// The only modification made to the reference implementation, because OCaml
// only supports double precision floating points number, meaning the generated
// numbers are "even more precise".
#define float double
...
constexpr float Get1dNoiseZeroToOne( int index, unsigned int seed )
{
	...
	return (double)( ONE_OVER_MAX_UINT * (double) SquirrelNoise5( index, seed ) );
}
